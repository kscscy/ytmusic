...
$(function(event) {
	...
	s4.src = "js/album/jquery.blockUI.js";
...
$(document.body).on('click', '#deleteAllCheck',
		function(event) {
	console.log(".deleteAllCheck");
			...
			console.log("all? : "+ $("input:checkbox[id='allCheck']").is(":checked"));
...
$(document.body).on('click', '.deleteMusicBtn',
		function(event) {
	 console.log(".deleteMusicBtn");
...
//$('#updateList').one("click",
$(document.body).on('click', '#updateList',
		...
		console.log("updateList");
			if(sessionStorage.getItem('no_array_data')==null){
				...
				var music_id = null;
				var num = 0;
				console.log("앨범:"+album_no);
				//console.log(seq_session);
				//console.log(seq_session.length);

				if(result){
					var num_2 = 0;

					if(num==0) {
						console.log("앨범노래전체지우기 ㅡ " );

						$.ajax({
							url: 'album/deleteallmusic.do',
							type: 'POST',
							async: false,
							data: {
								album_no : album_no,
...
/////////////////////////////////////
					...
					if(num==1) {
						for (var i=0; i< seq_session.length; i++) {
							music_id = seq_session[i];
							console.log("insert : " + music_id);

							$.ajax({
								url: 'album/addmusic.do',
								type: 'POST',
								async: false,
								data: {
									album_no : album_no,
...
$(document.body).on('click', '#allCheck',
		function(event) {
