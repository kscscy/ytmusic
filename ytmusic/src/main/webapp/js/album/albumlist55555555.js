...
*/
	...
	function clickDetailLnk(event) {
		...
		*/
		...
		//$.blockUI({ message: '<h1>로딩중??????...</h1>' });
		var c = function(album_no) {
			$.ajax({
			...
			if (resultObj.data != null) {
				for (music of resultObj.data) {
					...
					.html("<td><input value='" + music.id + "'id='checkMusic' type='checkbox'/>" + 
							...
							"</td><td>" + music.title +
							"</td><td> <button id='"+ music.id +"' class='deleteMusicBtn' type='button' class='btn btn-warning btn-xs'>삭제</button>" + 
					"</td></tr>")
					.appendTo(table);
...
/////////////////////////////////////
						...
						//sessionStorage.setItem('no_array_data',null);
